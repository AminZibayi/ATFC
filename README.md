...
## Keyword Canonicalization Workflow

The project includes an automated pipeline (`canonicalization-workflow`) to normalize and merge author keywords against curated ontologies and glossaries (e.g., the Artificial Intelligence Ontology (AIO) and blockchain taxonomies).

This workflow runs independently of the main ETL pipeline and produces canonical mappings for keywords.

### Available Targets

```bash
# 1. Fetch external vocabularies (e.g., AIO from GitHub)
pnpm nx run canonicalization-workflow:fetch-vocabularies

# 2. Canonicalize keywords
# This stage extracts keywords from the intermediate `records.parquet` generated by the `bibliometric-pipeline`,
# normalizes them, and attempts exact and fuzzy matching (via RapidFuzz) against the combined AI/Blockchain master vocabulary.
# It outputs `keyword_synonyms.csv` and a `manual_review.csv` file for unmapped terms.
pnpm nx run canonicalization-workflow:canonicalize-keywords
```
